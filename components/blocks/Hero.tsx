...
export function Hero() {
  return (
    <section className="relative overflow-hidden bg-[radial-gradient(1200px_600px_at_110%_-10%,rgba(33,150,214,0.18),transparent_60%),radial-gradient(800px_500px_at_-10%_110%,rgba(130,189,63,0.10),transparent_60%),linear-gradient(180deg,#FAFCFE_0%,#FFFFFF_100%)] py-[clamp(56px,8vw,110px)]">
      <div className="mx-auto grid max-w-[1240px] grid-cols-[1.05fr_1fr] items-start gap-[clamp(32px,6vw,80px)] px-[clamp(20px,4vw,56px)] max-[900px]:grid-cols-1">
        {/* ── Copy ── */}
        <div>
          <span className="inline-flex items-center gap-2 text-[13px] font-semibold tracking-[0.14em] text-[#1779B8] uppercase before:inline-block before:h-0.5 before:w-6 before:bg-[#2196D6]">
            Your comfort and safety — sorted fast
          </span>
          ...
          >
            Feel safe. Stay <span className="text-[#1779B8]">comfortable</span>. Trust the{' '}
            <span className="text-[#E73438]">experts</span>.
          </Heading>

          <P className="mt-6 max-w-[560px] text-[clamp(16px,1.2vw,19px)] leading-[1.6] text-[#4F6172]">
            Flickering lights keeping you up at night? AC struggling through another Brisbane
            summer? You shouldn&apos;t have to live with it. Since 2014 we&apos;ve helped thousands
            of homeowners across Brisbane and the Gold Coast get the comfort and safety they deserve
            — with upfront quotes, no hidden fees, and a workmanship guarantee on every job.
          </P>
